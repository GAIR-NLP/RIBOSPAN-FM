...
SUPPORTED_BACKENDS = ("rnabert", "ribospan", "hydrarna")
HF_DIR_BACKENDS = ("rnabert", "ribospan")
YARN_BACKENDS = ("rnabert", "ribospan")


def hydrarna_runtime_available() -> tuple[bool, str]:
    """Return whether this process can import the HydraRNA stack."""

    try:
        import mamba_ssm  # noqa: F401
    except ImportError:
        return False, (
            "mamba_ssm is not importable; run HydraRNA cosine in the benchmark "
            "conda environment"
        )
    return True, ""
    ...
    *,
    ...
    for name, value in parsed:
        ...
        if raw_backend not in SUPPORTED_BACKENDS:
            raise ValueError(
                f"Registry model {name!r} has unsupported backend {raw_backend!r}"
            )
        ...
        if raw_backend not in YARN_BACKENDS and rope_scaling is not None:
            raise ValueError(
                f"Registry model {name!r} backend {raw_backend} does not "
                "support rope_scaling"
            )
...
def load_hf_mlm_checkpoint(
    ...
    *,
    ...
    model._benchmark_original_max_position_embeddings = config_max
    model._benchmark_runtime_max_position_embeddings = runtime_max
    model._benchmark_autocast_dtype = dtype
...
def load_encoder(
    ...
    *,
    ...
    """Load the encoder backend declared by ``spec.backend``.

    HydraRNA is returned as a duck-typed object with the same
    ``hidden_states`` / ``close`` surface as ``LoadedRNABert``.
    """

    backend = spec.backend or "rnabert"
    if backend in HF_DIR_BACKENDS:
        return load_hf_masked_lm(
            spec,
            device=device,
            dtype=dtype,
    ...
    if backend == "hydrarna":
        from .hydrarna_io import load_hydrarna
