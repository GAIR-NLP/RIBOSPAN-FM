...
"""HydraRNA encoder: last-layer walk with the shared ``embed_pooled`` contract."""
...
from scripts.model_io import ModelSpec, normalize_sequence
from scripts.model_src import HYDRARNA_SRC_ROOT
...
class LoadedHydraRNA:
    """Fairseq HydraRNA encoder with the clustering ``embed_pooled`` surface."""
    ...
    def hidden_size(self) -> int:
        ...
        embed = getattr(encoder, "embed_tokens", None)
        if embed is not None and hasattr(embed, "embedding_dim"):
            return int(embed.embedding_dim)
        args = getattr(encoder, "args", None)
        for key in ("encoder_embed_dim", "decoder_embed_dim", "embed_dim"):
            value = getattr(args, key, None) if args is not None else None
            if value:
                return int(value)
        raise RuntimeError(f"{self.spec.name}: cannot infer hidden_size")

    def _layer_states(self, sequence: str) -> tuple[torch.Tensor, ...]:
        """Return embedding + per-block states including BOS/EOS, on GPU."""
        ...
        expected = len(normalized) + 2
        if any(int(state.shape[1]) != expected for state in hidden):
            lengths = [int(state.shape[1]) for state in hidden]
            raise RuntimeError(
                "HydraRNA tokenization produced unexpected lengths "
                f"{lengths} for RNA length {len(normalized)} (expected {expected})"
            )
        return hidden
    ...
    def embed_pooled(
        ...
        *,
        ...
        """Pool ``<s> + bases + EOS`` the same way as RNABert / RNA-FM."""

        from scripts.backends import _pool_cls_body

        hidden = self._layer_states(sequence)
        n_states = len(hidden)
        vectors: dict[int, dict[str, np.ndarray]] = {}
        for layer in layers:
            if layer < 0 or layer >= n_states:
                raise ValueError(
                    f"{self.spec.name}: layer {layer} out of range for "
                    f"{n_states} states (0..{n_states - 1})"
                )
            # [1, T, D] -> [T, D]; layout is <s> + bases + EOS
            state = hidden[layer][0]
            layer_vecs: dict[str, np.ndarray] = {}
            for pooling in poolings:
                pooled = _pool_cls_body(state, pooling=pooling)
                layer_vecs[pooling] = (
...
def resolve_hydrarna_root(spec: ModelSpec | None = None) -> Path:
    ...
    for candidate in candidates:
        ...
        if (candidate / "fairseq").is_dir() and (
            candidate / "fairseq" / "models"
        ).is_dir():
            return candidate.resolve()
...
def _encoder_hidden_states(
    encoder: Any, src_tokens: torch.Tensor
) -> tuple[torch.Tensor, ...]:
